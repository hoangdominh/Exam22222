...
//   Nhập ID học sinh cho xóa thông tin học sinh
    ...
    public void timHocSinh(String hoVaTen){
        for (HocSinh hocSinh : hocSinhList){
            if(hocSinh.getHoVaTen().equalsIgnoreCase(hoVaTen)){
                System.out.println(hocSinh);
            }
        }
    }
    ...
    public void inDanhSachXepSepTheoTen(){
        Collections.sort(this.hocSinhList, new SapXep() {
//            @Override
...
//  Nhập vào năm sinh, in ra danh sách học sinh trong năm đó
    ...
    public void timNamSinh(int year){
//        for (HocSinh hocSinh :hocSinhList){
//            if (hocSinh.getNgaySinh().indexOf(namSinh)>=0){
//                System.out.println(hocSinh);
//            }
//        }
        ArrayList<HocSinh> namSinh = new ArrayList<HocSinh>();
        for (int i = 0; i < hocSinhList.size(); i++){
            Calendar cL = Calendar.getInstance();
            cL.setTime(hocSinhList.get(i).getNgaySinh());
            int yearB = cL.get(year);
            if (yearB == year){
                namSinh.add(hocSinhList.get(i));
            }
        }
        for (HocSinh o : namSinh){
